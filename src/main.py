...
class Emails:
    ...
    # task2 method
    def search_by_text(self, string): # (--search str, -s str)
        l = list()
        for file_name in os.listdir(self.directory):
            absolutePath = self.directory + '\\' + file_name
            # opening *.txt files
    ...
    # task3 method
    def group_by_domain(self): # (--group-by-domain, -gbd)
        d = dict()
        for file_name in os.listdir(self.directory):
            absolutePath = self.directory + '\\' + file_name
            # opening *.txt files
...
def check_input(num, basic='integer'):
    ...
    while True:
        ...
        #task2
            ...
            while True:
                ...
                if check_input(user_choice, basic='string') == False:
                    continue
                break

            print("Found emails with user phrase in email: "
                  + str(len(emails_object.search_by_text(user_choice))))
            print(*emails_object.search_by_text(user_choice), sep='\n')

        #task3
        elif(int(x) == 3):
            print('Number of occurrences of each email address')
            print(emails_object.group_by_domain())

        #task4
        elif(int(x) == 4):
            print('Emails not sent: '
                  + str(len(emails_object.find_not_in_logs())))
            print(*sorted(emails_object.find_not_in_logs()), sep='\n')
